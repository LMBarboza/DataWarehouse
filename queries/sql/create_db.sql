...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conname = 'fk_veiculo_vagas' AND conrelid = 'public.veiculo'::regclass
    ) THEN
        ALTER TABLE public.VEICULO
        ADD CONSTRAINT FK_VEICULO_VAGAS FOREIGN KEY (ID_VAGAS)
        REFERENCES public.VAGAS(ID_VAGAS) ON DELETE SET NULL ON UPDATE CASCADE;
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'fk_estado_veiculo_locacao' AND conrelid = 'public.estado_veiculo'::regclass
    ) THEN
        ALTER TABLE public.ESTADO_VEICULO
        ADD CONSTRAINT FK_ESTADO_VEICULO_LOCACAO FOREIGN KEY (ID_LOCACAO)
        REFERENCES public.LOCACAO(ID_LOCACAO) ON DELETE SET NULL ON UPDATE CASCADE;
    END IF;
END $$;
